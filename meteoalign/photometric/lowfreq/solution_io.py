...
def solution_to_payload(solution: PhotometricSolution) -> dict[str, Any]:
    ...
    return {
        ...
        "correction_domain": "camera_raw_tiff_code_value",
        "model_type": "additive_bspline",
        "grid": {
        ...
        "channels": {
            channel_name: {"coefficients": _float_list(solution.coefficients_rgb[channel])}
...
def read_solution_payload(path: str | Path) -> dict[str, Any]:
    ...
    if payload.get("schema") != SOLUTION_SCHEMA or int(payload.get("version", 0)) != SOLUTION_VERSION:
...
def read_solution(path: str | Path) -> PhotometricSolution:
    ...
    coefficients = np.asarray(
        [
            channels.get(channel_name, {}).get("coefficients", [])
